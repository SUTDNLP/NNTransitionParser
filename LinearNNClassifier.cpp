...
LinearNNClassifier::LinearNNClassifier() {
...
// did not judge memory allocate fail!
void LinearNNClassifier::init(int wordDim, int wordSize, int wordcontext, int atomDim, int atomSize, int atomcontext, int lay1OutDim, int labelSize,
    int linearfeatSize) {
  _b_wordEmb_finetune = true;
...
// did not judge memory allocate fail!
void LinearNNClassifier::init(const mat& wordEmb, int wordcontext, int atomDim, int atomSize, int atomcontext, int lay1OutDim, int labelSize,
    int linearfeatSize) {
...
void LinearNNClassifier::preCompute() {
...
double LinearNNClassifier::process(const vector<Example>& examples, int iter) {
  _eval.reset();
  ...
  for (count = 0; count < example_num; count++) {
    ...
    output = layer2out + layer_linear_output;
    ...
    _layer_linear.ComputeBackwardLoss(linear_features, layer2outLoss);
...
void LinearNNClassifier::predict(const Feature& feature, vector<double>& results) {
...
double LinearNNClassifier::computeScore(const Example& example) {
  NRVec<double> mid_layer1out(_layer1OutDim);
  mat layer1out;
  mat layer2out;
  mat layer_linear_output, output;
...
void LinearNNClassifier::updateParams(double regularizationWeight, double adaAlpha, double adaEps) {
...
void LinearNNClassifier::checkgradColSparse(const vector<Example>& examples, mat& Wd, const mat& gradWd, const string& mark, int iter,
...
void LinearNNClassifier::checkgradRowSparse(const vector<Example>& examples, mat& Wd, const mat& gradWd, const string& mark, int iter,
...
void LinearNNClassifier::checkgrad(const vector<Example>& examples, mat& Wd, const mat& gradWd, const string& mark, int iter) {
  //Random randWdRowcheck = new Random(iter + "Row".hashCode() + hash));
  int charseed = mark.length();
  for (int i = 0; i < mark.length(); i++) {
    charseed = (int) (mark[i]) * 5 + charseed;
...
void LinearNNClassifier::checkgrad(const vector<Example>& examples, double* Wd, const double* gradWd, const string& mark, int iter, int rowSize, int colSize) {
  //Random randWdRowcheck = new Random(iter + "Row".hashCode() + hash));
  int charseed = mark.length();
  for (int i = 0; i < mark.length(); i++) {
    charseed = (int) (mark[i]) * 5 + charseed;
...
void LinearNNClassifier::checkgrads(const vector<Example>& examples, int iter) {
