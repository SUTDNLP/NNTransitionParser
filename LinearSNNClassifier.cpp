...
LinearSNNClassifier::LinearSNNClassifier() {
...
// did not judge memory allocate fail!
void LinearSNNClassifier::init(int wordDim, int wordSize, int wordcontext, int atomDim, int atomSize, int atomcontext, int labelSize,
    int linearfeatSize) {
  _b_wordEmb_finetune = true;
...
// did not judge memory allocate fail!
void LinearSNNClassifier::init(const mat& wordEmb, int wordcontext, int atomDim, int atomSize, int atomcontext, int labelSize,
    int linearfeatSize) {
...
void LinearSNNClassifier::preCompute() {
  ...
  _atomPreComputedForward = (double *) calloc(_actionSize * _curAtomPreComputedNum, sizeof(double));
  count = 0;
...
double LinearSNNClassifier::process(const vector<Example>& examples, int iter) {
  _eval.reset();
  ...
  _atomPreComputedBackward = (double *) calloc(_actionSize * _curAtomPreComputedNum, sizeof(double));
...
void LinearSNNClassifier::predict(const Feature& feature, vector<double>& results) {
...
double LinearSNNClassifier::computeScore(const Example& example) {
  NRVec<double> mid_layer1out(_actionSize);
  mat layer1out;
  mat layer_linear_output, output;
...
void LinearSNNClassifier::updateParams(double regularizationWeight, double adaAlpha, double adaEps) {
...
void LinearSNNClassifier::checkgradColSparse(const vector<Example>& examples, mat& Wd, const mat& gradWd, const string& mark, int iter,
...
void LinearSNNClassifier::checkgrad(const vector<Example>& examples, mat& Wd, const mat& gradWd, const string& mark, int iter) {
  //Random randWdRowcheck = new Random(iter + "Row".hashCode() + hash));
  int charseed = mark.length();
  for (int i = 0; i < mark.length(); i++) {
    charseed = (int) (mark[i]) * 5 + charseed;
...
void LinearSNNClassifier::checkgrads(const vector<Example>& examples, int iter) {
